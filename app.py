...
for ticker in tickers:
    ...
    def safe_metric(val): return f"{float(val):.2f}" if pd.notna(val) else "N/A"
    def safe_int(val): return int(val) if pd.notna(val) else "N/A"
    ...
    # Subplot chart: Candlestick + RSI + MACD
